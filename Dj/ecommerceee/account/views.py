...
def register(request):
    if request.method == 'POST': 
        ...
        if form.is_valid():
            ...
            # send_email.send()
            messages.success(request,'A mail is send to the given email id. Please verify your identity')
            #  this will take email to login page dynamically to provide better user experience
    ...
    if request.method == 'POST':
        ...
        if user is not None:
            try:
                ...
                is_cart_item_ex = CartItem.objects.filter(cart=cart).exists()
                if is_cart_item_ex:
